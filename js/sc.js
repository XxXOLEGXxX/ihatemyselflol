const SOFTCAPS = { //HALVEDLOL
	...
	hindr_base: {
		...
		mag() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("h"):false)?3.25:4) },
	...
	qe: {
		...
		start() { 
			...
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("q"):false) start = start.pow(1.125);
			return start;
		},
		mag: new Decimal(2),
	...
	hex: {
		title: "Hex Effect",
		type: "log",
		start() { return new Decimal("1e10000").times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("m"):false)?Decimal.pow(1.000005, player.m.points.div(2).plus(1).log10()):1) },
		exp() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("m"):false)?1e3:10) },
	...
	hsBuilds: {
		...
		type: "root",
		start() { return Decimal.add(3, hasAchievement("a", 121)?player.hs.buyables[11].div(2).root(5).times(.1):0).plus(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hs"):false)?.05:0) },
		mag: new Decimal(5),
...
const STATIC_SCALE_DATA = [
	{
		...
		start_adj: {
			"2": function() {
				...
				if (hasUpgrade("t", 34) && player.i.buyables[12].gte(4)) start = new Decimal(1312);
