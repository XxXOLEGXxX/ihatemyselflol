addLayer("p", { //HALVEDLOL
        ...
        exponent() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.625:0.5 }, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses //HALVEDLOL
            mult = new Decimal(1)
			if (hasAchievement("a", 13)) mult = mult.times(1.05); //HALVEDLOL
			if (hasAchievement("a", 32)) mult = mult.times(1.5); //HALVEDLOL
			if (hasUpgrade("p", 21)) mult = mult.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1e25:1.4); //HALVEDLOL
			if (hasUpgrade("p", 23)) mult = mult.times(upgradeEffect("p", 23)); //HALVEDLOL
			if (hasUpgrade("p", 41)) mult = mult.times(upgradeEffect("p", 41)); //HALVEDLOL
			if (hasUpgrade("b", 11)) mult = mult.times(upgradeEffect("b", 11)); //HALVEDLOL
			if (hasUpgrade("g", 11)) mult = mult.times(upgradeEffect("g", 11)); //HALVEDLOL
			...
			if (hasUpgrade("b", 31)) mult = mult.times(upgradeEffect("b", 31)); //HALVEDLOL
            return mult
        },
        gainExp() { // Calculate the exponent on main currency from bonuses //HALVEDLOL
            let exp = new Decimal(1)
			if (hasUpgrade("p", 31)) exp = exp.times(1.025);
			return exp;
        },
		...
		passiveGeneration() { return (hasMilestone("g", 1)&&player.ma.current!="p")?0.5:0 }, //HALVEDLOL
		...
		upgrades: {
			...
			11: { //HALVEDLOL
				title: "Begin",
				description: "Generate 0.5 Point every second.",
			...
			12: { //HALVEDLOL
				...
				effect() {
					...
					let eff = player.p.points.div(2).plus(1).pow(0.5); //HALVEDLOL
					if (hasUpgrade("g", 14)) eff = eff.pow(1.25); //HALVEDLOL
					if (hasUpgrade("g", 24)) eff = eff.pow(1.2333333); //HALVEDLOL
					if (hasUpgrade("g", 34) && player.i.buyables[12].gte(2)) eff = eff.pow(1.2166665) //HALVEDLOL
					
					if (hasChallenge("h", 22)) eff = softcap("p12_h22", eff); //uhh
					else eff = softcap("p12", eff); //uhh
					
					if (hasUpgrade("p", 14)) eff = eff.pow(2); //HALVEDLOL
					if (hasUpgrade("hn", 14)) eff = eff.pow(1.025); //HALVEDLOL
					if (hasUpgrade("b", 34) && player.i.buyables[12].gte(1)) eff = eff.pow(upgradeEffect("b", 34)); //HALVEDLOL
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) eff = eff.pow(1.05); //HALVEDLOL
				...
				formula() { 
					...
					let exp = new Decimal(0.5*(hasUpgrade("g", 14)?1.5:1)*(hasUpgrade("g", 24)?1.2333333:1)); //HALVEDLOL
					if (hasUpgrade("g", 34) && player.i.buyables[12].gte(2)) exp = exp.times(1.2166665); //HALVEDLOL
					if (hasUpgrade("b", 34) && player.i.buyables[12].gte(1)) exp = exp.times(upgradeEffect("b", 34));
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) exp = exp.times(1.05); //HALVEDLOL
					let f = "(x/2+1)^"+format(exp)
					if (upgradeEffect("p", 12).gte("1e3500")) { //uhh
						if (hasChallenge("h", 22)) f = "10^(sqrt(log(x+2))*"+format(Decimal.mul(exp, 3500).sqrt())+")" //uhh
						else f = "log(x+2)*"+format(Decimal.div("1e3500",3500).times(exp)) //fuck no
					}
					if (hasUpgrade("p", 14)) f += "^"+(hasUpgrade("hn", 14)?2.05:2) //HALVEDLOL
					return f;
				},
			},
			13: { //HALVEDLOL
				...
				effect() { 
					let eff = player.points.div(2).plus(1).log10().pow(0.75).plus(1); //HALVEDLOL
					if (hasUpgrade("p", 33)) eff = eff.pow(upgradeEffect("p", 33)); //HALVEDLOL
					if (hasUpgrade("g", 15)) eff = eff.pow(upgradeEffect("g", 15)); //HALVEDLOL
					if (hasUpgrade("hn", 13)) eff = eff.pow(upgradeEffect("hn", 13)); //HALVEDLOL
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) eff = eff.pow(38); //HALVEDLOL
					return eff;
				},
				...
				formula() { 
					let exp = new Decimal(1);
					if (hasUpgrade("p", 33)) exp = exp.times(upgradeEffect("p", 33)); //HALVEDLOL
					if (hasUpgrade("g", 15)) exp = exp.times(upgradeEffect("g", 15)); //HALVEDLOL
					if (hasUpgrade("hn", 13)) exp = exp.times(upgradeEffect("hn", 13)); //HALVEDLOL
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) exp = exp.times(38); //HALVEDLOL
					return "(log(x/2+1)^0.75+1)"+(exp.gt(1)?("^"+format(exp)):"") //HALVEDLOL
				},
			},
			14: { //HALVEDLOL
				title: "Prestigious Intensity",
				description: "<b>Prestige Boost</b>'s effect is squared (unaffected by softcap).",
			...
			21: { //HALVEDLOL
				title: "More Prestige",
				description() { return "Prestige Point gain is increased by "+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e27":"40")+"%." },
			...
			22: { //HALVEDLOL
				...
				effect() {
					let eff = Decimal.pow(1.2, player.p.upgrades.length/2); //HALVEDLOL
					if (hasUpgrade("p", 32)) eff = eff.pow(1.5); //HALVEDLOL
					if (hasUpgrade("hn", 22)) eff = eff.pow(upgradeEffect("hn", 22)) //HALVEDLOL
					if (hasUpgrade("hn", 32)) eff = eff.pow(4); //HALVEDLOL
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) eff = eff.pow(20); //HALVEDLOL
					return eff;
				},
				unlocked() { return hasAchievement("a", 21)&&hasUpgrade("p", 12) },
				effectDisplay() { return format(tmp.p.upgrades[22].effect)+"x" },
				formula() { 
					let exp = new Decimal(hasUpgrade("p", 32)?1.5:1); //HALVEDLOL
					if (hasUpgrade("hn", 22)) exp = exp.times(upgradeEffect("hn", 22)); //HALVEDLOL
					if (hasUpgrade("hn", 32)) exp = exp.times(4); //HALVEDLOL
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) exp = exp.times(20); //HALVEDLOL
					return exp.gt(1)?("(1.2^(x/2))^"+format(exp)):"1.2^(x/2)"  //HALVEDLOL
				},
			},
			23: { //HALVEDLOL
				...
				effect() {
					let eff = player.points.div(2).plus(1).log10().cbrt().plus(1); //HALVEDLOL
					if (hasUpgrade("p", 33)) eff = eff.pow(upgradeEffect("p", 33)); //HALVEDLOL
					if (hasUpgrade("g", 23)) eff = eff.pow(upgradeEffect("g", 23)); //HALVEDLOL
					if (hasUpgrade("hn", 23)) eff = eff.pow(upgradeEffect("hn", 23)); //HALVEDLOL
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) eff = eff.pow(1.25); //HALVEDLOL
					return eff;
				},
				...
				formula() { 
					let exp = new Decimal(1);
					if (hasUpgrade("p", 33)) exp = exp.times(upgradeEffect("p", 33)); //HALVEDLOL
					if (hasUpgrade("g", 23)) exp = exp.times(upgradeEffect("g", 23)); //HALVEDLOL
					if (hasUpgrade("hn", 23)) exp = exp.times(upgradeEffect("hn", 23)); //HALVEDLOL
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) exp = exp.times(1.25); //HALVEDLOL
					return exp.gt(1)?("(log(x/2+1)^(1/3)+1)^"+format(exp)):"log(x/2+1)^(1/3)+1"
				},
			},
			24: { //HALVEDLOL
				title: "Plasmic Energies",
				description: "The Tachoclinal Plasma effect uses a better formula (log(log(x/2+1)+1)*5+1 -> 5^cbrt(log(x/2+1))).",
			...
			31: { //HALVEDLOL
				...
				description: "Prestige Point gain is raised to the power of 1.025.",
			...
			32: { //HALVEDLOL
				title: "Still Useless",
				description: "<b>Upgrade Power</b> is ^1.5'd.",
			...
			33: { //HALVEDLOL
				...
				formula() { return hasUpgrade("hn", 33) ? ("(log(log(x/2+1)+1)/5+1)*"+format(upgradeEffect("hn", 33))) : "log(log(x/2+1)+1)/5+1" },
			},
			34: { //HALVEDLOL
				...
				unlocked() { return player.p.pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.o.points.div(2).plus(1).log10().plus(1).log10().plus(1).log10().plus(1).times((hasUpgrade("hn", 34)) ? upgradeEffect("hn", 34) : 1) },
				effectDisplay() { return format(tmp.p.upgrades[34].effect)+"x" },
				formula: "log(log(log(x/2+1)+1)+1)+1",
			},
			41: { //HALVEDLOL
				...
				unlocked() { return player.p.pseudoUpgs.includes(Number(this.id)) },
				effect() { 
					let eff = Decimal.pow(5, player.p.points.div(2).plus(1).log10().pow(.8));
					if (hasUpgrade("hn", 41)) eff = eff.pow(upgradeEffect("hn", 41));
					return eff;
				},
				effectDisplay() { return format(tmp.p.upgrades[41].effect)+"x" },
				formula() { return "5^(log(x/2+1)^0.8)"+(hasUpgrade("hn", 41)?("^"+format(upgradeEffect("hn", 41))):"") },
			},
			42: { //HALVEDLOL
				title: "Spatial Awareness",
				description: "Space Building costs scale 25% slower.",
			...
			44: { //HALVEDLOL
				...
				unlocked() { return player.p.pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.b.points.div(2).plus(1).pow(2) },
...
addLayer("b", { //HALVEDLOL
		...
		branches: ["p"],
        exponent() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1:1.25 }, // Prestige currency exponent //HALVEDLOL
		base() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?3.25:5 }, //HALVEDLOL
		gainMult() {  //HALVEDLOL
			let mult = new Decimal(1);
			if (hasUpgrade("b", 23)) mult = mult.div(upgradeEffect("b", 23)); //HALVEDLOL
			if (player.s.unlocked) mult = mult.div(buyableEffect("s", 13)); //HALVEDLOL
			return mult;
		},
		...
		addToBase() { //HALVEDLOL
			...
			if (hasUpgrade("b", 12)) base = base.plus(upgradeEffect("b", 12)); //HALVEDLOL
			if (hasUpgrade("b", 13)) base = base.plus(upgradeEffect("b", 13)); //HALVEDLOL
			if (hasUpgrade("t", 11)) base = base.plus(upgradeEffect("t", 11)); //HALVEDLOL
			if (hasUpgrade("e", 11)) base = base.plus(upgradeEffect("e", 11).b); //HALVEDLOL
			if (player.e.unlocked) base = base.plus(layers.e.buyables[11].effect().second); //HALVEDLOL
			if (player.s.unlocked) base = base.plus(buyableEffect("s", 12)); //HALVEDLOL
			if (hasUpgrade("t", 25)) base = base.plus(upgradeEffect("t", 25)); //HALVEDLOL
			return base;
		},
		effectBase() { //HALVEDLOL
		...
		power() { //HALVEDLOL
			...
			if (player.m.unlocked) power = power.times(player.m.spellTimes[12].gt(0)?1.025:1);
			return power;
		},
		effect() { //HALVEDLOL
		...
		upgrades: {
			...
			11: { //HALVEDLOL
				...
				effect() { 
					let ret = player.b.best.div(2).sqrt().plus(1); //HALVEDLOL
					if (hasUpgrade("b", 32)) ret = Decimal.pow(1.0625, player.b.best.div(2)).times(ret); //HALVEDLOL
					if (hasUpgrade("s", 15)) ret = ret.pow(buyableEffect("s", 14).root(2.7)); //HALVEDLOL
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) ret = ret.pow(upgradeEffect("b", 14)); //HALVEDLOL
					if (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)) ret = ret.pow(1.25); //HALVEDLOL
					return ret;
				},
				unlocked() { return player.b.unlocked },
				effectDisplay() { return format(tmp.b.upgrades[11].effect)+"x" },
				formula() { 
					let base = "sqrt(x/2)+1" //HALVEDLOL
					if (hasUpgrade("b", 32)) base = "(sqrt(x/2)+1)*(1.0625^x/2)" //HALVEDLOL
					let exp = new Decimal(1)
					if (hasUpgrade("s", 15)) exp = exp.times(buyableEffect("s", 14).root(2.7)); //HALVEDLOL
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) exp = exp.times(upgradeEffect("b", 14)); //HALVEDLOL
					if (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)) exp = exp.times(1.25); //HALVEDLOL
			...
			12: { //HALVEDLOL
				...
				effect() {
					let ret = player.g.points.div(2).add(1).log10().sqrt().div(3).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1);
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) ret = ret.pow(upgradeEffect("b", 14)); //HALVEDLOL
					return ret;
				},
				...
				formula() { 
					let exp = new Decimal(1);
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) exp = exp.times(upgradeEffect("b", 14)); //HALVEDLOL
					let f = "sqrt(log(x/2+1))"+(hasUpgrade("e", 14)?("*"+format(upgradeEffect("e", 14).div(3))):"/3") 
			...
			13: { //HALVEDLOL
				...
				effect() { 
					let ret = player.p.total.div(2).add(1).log10().add(1).log10().div(3).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1) 
				...
				formula() { 
					...
					let f = "log(log(x/2+1)+1)"+(hasUpgrade("e", 14)?("*"+format(upgradeEffect("e", 14).div(3))):"/3") 
					if (exp.gt(1)) f = "("+f+")^"+format(exp);
					return f;
				},
			},
			14: { //HALVEDLOL
				...
				effect() { return player.sb.points.div(2).plus(1) },
			...
			21: { //HALVEDLOL
				title: "Gen Z^1.5",
				description: "^1.5 the Generator Power effect.",
			...
			23: { //HALVEDLOL
				...
				effect() { 
					let ret = player.points.div(2).add(1).log10().add(1).pow(2.1);  //HALVEDLOL
					if (player.s.unlocked) ret = ret.pow(buyableEffect("s", 14));  //HALVEDLOL
					if (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)) ret = ret.pow(1.25); //HALVEDLOL
					return ret;
				},
				unlocked() { return hasUpgrade("b", 21) || hasUpgrade("b", 22) },
				effectDisplay() { return "/"+format(tmp.b.upgrades[23].effect) },
				formula() { return "(log(x/2+1)+1)^"+(player.s.unlocked?format(buyableEffect("s", 14).times(2.1).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.25:1)):"2.1") },
			},
			24: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.b.points.div(2).plus(1).pow(250) },
			...
			31: { //HALVEDLOL
				...
				formula() { 
					let exp = ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1e4:1
					return "1e10^(x/2^1.25)"+(exp==1?"":("^"+format(exp)));
				},
			},
			32: { //HALVEDLOL
				title: "Better BP Combo",
				description() { return "<b>BP Combo</b> uses a better formula"+(tmp.nerdMode?" (sqrt(x/2+1) -> (1.0625^x/2)*sqrt(x/2+1))":"")+"." },
			...
			33: { //HALVEDLOL
				...
				formula() { 
					...
					let f = "x/2*"+(player.sb.points.gte(4)?"1.8":"1.5")+"+1"
					if (exp==1) return f;
			...
			34: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.i.points.div(2).plus(1).root(4) },
				effectDisplay() { return "^"+format(tmp[this.layer].upgrades[this.id].effect) },
				formula: "(x/2+1)^0.25",
			},
		},
...
addLayer("g", { //HALVEDLOL
		...
		effect() { //HALVEDLOL
			...
			let eff = Decimal.pow(this.effBase(), player.g.points.div(2).plus(tmp.sg.spectralTotal)).sub(1).max(0);
		...
		powerExp() { //HALVEDLOL
			...
			if (hasUpgrade("b", 21)) exp = exp.times(1.5); //HALVEDLOL
			if (hasUpgrade("b", 22)) exp = exp.times(1.1); //HALVEDLOL
			if (hasUpgrade("q", 13)) exp = exp.times(1.125); //HALVEDLOL
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) exp = exp.times(1.025); //HALVEDLOL
			if (player.mc.upgrades.includes(11)) exp = exp.times(buyableEffect("mc", 12));
			if (hasAchievement("a", 152)) exp = exp.times(1.2); //HALVEDLOL
			return exp;
		},
		powerEff() { //HALVEDLOL
			if (!unl(this.layer)) return new Decimal(1);
			return player.g.power.div(2).plus(1).pow(this.powerExp());
		},
		doReset(resettingLayer) {
			let keep = [];
		...
		upgrades: {
			...
			11: {
				...
				effect() { return player.g.best.div(2).sqrt().plus(1).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2.5e5:1) }, //halved ig???
				unlocked() { return player.g.unlocked },
				effectDisplay() { return format(tmp.g.upgrades[11].effect)+"x" },
				formula() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"(x/2+1)^250,000":"sqrt(x/2)+1" }, //what
			},
			12: { //HALVEDLOL
				title: "I Need More!",
				...
				effect() { 
					let ret = player.b.points.div(2).add(1).log10().sqrt().div(3).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1);
				...
				formula() { 
					...
					return "sqrt(log(x/2+1))"+(m.eq(1)?"":(m.gt(1)?("*"+format(m)):("/"+format(m.pow(-1)))));
				},
			},
			13: { //HALVEDLOL
				...
				effect() { 
					let ret = player.p.best.div(2).add(1).log10().add(1).log10().div(3).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1);
					if (hasUpgrade("s", 24)) ret = ret.times(upgradeEffect("s", 24));
					return ret;
				},
				...
				formula() { 
					let m = new Decimal(hasUpgrade("e", 14)?upgradeEffect("e", 14):1).div(3)
					if (hasUpgrade("s", 24)) m = upgradeEffect("s", 24).times(m);
					return "log(log(x/2+1)+1)"+(m.eq(1)?"":(m.gt(1)?("*"+format(m)):("/"+format(m.pow(-1)))));
				},
			},
			14: { //HALVEDLOL
			...
			15: { //HALVEDLOL
				...
				effect() { 
					let eff = player.g.points.div(2).sqrt().add(1); //HALVEDLOL
					if (eff.gte(400)) eff = eff.cbrt().times(Math.pow(400, 2/3)) //i shouldn't probably touch that thing
					return eff;
				},
				unlocked() { return hasUpgrade("g", 13) },
				effectDisplay() { return "^"+format(tmp.g.upgrades[15].effect) },
				formula() { return upgradeEffect("g", 15).gte(400)?"((x/2+1)^(1/6))*(400^(2/3))":"sqrt(x/2)+1" }, 
			},
			21: { //HALVEDLOL
                ...
                currencyLayer: "g",
				effect() { 
					let ret = player.g.power.div(2).add(1).log10().add(1);
					if (hasUpgrade("s", 24)) ret = ret.pow(upgradeEffect("s", 24));
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) ret = ret.pow(5e3)
					return ret;
				},
				...
				formula() { 
					...
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) exp = exp.times(5e3);
					let f = "log(x/2+1)+1";
					if (exp.gt(1)) f = "("+f+")^"+format(exp);
					return f;
				},
			...
			22: { //HALVEDLOL
                ...
                currencyLayer: "g",
				effect() { 
					let eff = player.p.points.div(2).add(1).pow(0.25);
			...
			23: { //HALVEDLOL
                ...
                currencyInternalName: "power",
                currencyLayer: "g",
				effect() { return player.b.points.div(2).pow(0.85).add(1) },
			...
			24: { //HALVEDLOL
				...
				description: "<b>Prestige Boost</b> is raised to the power of 1.233.",
			...
			25: { //HALVEDLOL
                ...
                currencyLayer: "g",
				effect() { 
					let ret = player.p.points.div(2).add(1).log10().pow(2).add(1);
					if (hasUpgrade("s", 24)) ret = ret.pow(upgradeEffect("s", 24));
					return ret;
				},
			...
			31: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.g.power.div(2).plus(1).log10().plus(1).pow(1.5) },
				effectDisplay() { return format(tmp[this.layer].upgrades[this.id].effect)+"x" },
				formula: "(log(x/2+1)+1)^1.5",
			},
			32: { //HALVEDLOL
			...
			34: { //HALVEDLOL
				...
				description: "<b>Prestige Boost</b> is raised to the power of 1.216.",
			...
			35: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.g.power.div(2).plus(1).log10().plus(1).sqrt() },
				effectDisplay() { return format(tmp[this.layer].upgrades[this.id].effect)+"x" },
				formula: "sqrt(log(x/2+1)+1)",
			},
		},
})

addLayer("t", { //HALVEDLOL
        ...
        baseAmount() {return player.points}, // Get the current amount of baseResource
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?new Decimal(1.625):new Decimal(1.85) }, // Prestige currency exponent //HALVEDLOL
		base() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?new Decimal(1e8):new Decimal(1e15) }, //HALVEDLOL
		...
		enCapMult() { //HALVEDLOL
			let mult = new Decimal(1);
			if (hasUpgrade("t", 12)) mult = mult.times(upgradeEffect("t", 12)); //HALVEDLOL
			if (hasUpgrade("t", 21)) mult = mult.times(50); //HALVEDLOL
		...
		enGainMult() { //HALVEDLOL
			let mult = new Decimal(1);
			if (hasUpgrade("t", 22)) mult = mult.times(upgradeEffect("t", 22));
			if (player.h.unlocked) mult = mult.times(tmp.h.effect);
			return mult;
		},
		effBaseMult() { //HALVEDLOL
			let mult = new Decimal(1);
		...
		effBasePow() { //HALVEDLOL
			let exp = new Decimal(1);
			if (player.m.unlocked) exp = exp.times(player.m.spellTimes[12].gt(0)?1.05:1);
			return exp;
		},
		effGainBaseMult() { //HALVEDLOL
			let mult = new Decimal(1);
		...
		effect() { //HALVEDLOL
			if (!unl(this.layer)) return {gain: new Decimal(0), limit: new Decimal(0)};
				...
				gain: Decimal.pow(tmp.t.effBaseMult.times(tmp.t.effGainBaseMult).times(2).pow(tmp.t.effBasePow), player.t.points.div(2).times(tmp.t.nonExtraTCPow).plus(player.t.buyables[11].div(2)).plus(tmp.t.freeExtraTimeCapsules.div(2))).sub(1).max(0).times(player.t.points.div(2).times(tmp.t.nonExtraTCPow.div(2)).plus(player.t.buyables[11].div(2)).gt(0)?1:0).times(tmp.t.enGainMult).max(0),
				limit: Decimal.pow(tmp.t.effBaseMult.times(tmp.t.effLimBaseMult).times(1.5).pow(tmp.t.effBasePow), player.t.points.div(2).times(tmp.t.nonExtraTCPow).plus(player.t.buyables[11].div(2)).plus(tmp.t.freeExtraTimeCapsules.div(2))).sub(1).max(0).times(50).times(player.t.points.div(2).times(tmp.t.nonExtraTCPow.div(2)).plus(player.t.buyables[11].div(2)).gt(0)?1:0).times(tmp.t.enCapMult).max(0),
			}
		},
		effect2() { //HALVEDLOL
			if (!((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) || !unl(this.layer)) return new Decimal(1);
			let c = player.t.points.div(2).plus(player.t.buyables[11].div(2)).plus(tmp.t.freeExtraTimeCapsules.div(2));
			return Decimal.pow(1.005, c.div(2).sqrt());
		},
		effectDescription() {
			return "which are generating "+format(tmp.t.effect.gain)+" Time Energy/sec, but with a limit of "+format(tmp.t.effect.limit)+" Time Energy"+(tmp.nerdMode?("\n("+format(tmp.t.effBaseMult.times(tmp.t.effGainBaseMult).times(2))+"x gain each, "+format(tmp.t.effBaseMult.times(1.5))+"x limit each)"):"")+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?(", and which multiply the speed of all layers before Mastery by "+format(tmp.t.effect2)+(tmp.nerdMode?(" (1.005^sqrt(x/2))"):"")):"")
		},
		enEff() { //HALVEDLOL
			if (!unl(this.layer)) return new Decimal(1);
			let eff = player.t.energy.div(2).add(1).pow(1.1);
			if (hasUpgrade("t", 14)) eff = eff.pow(1.15);
			if (hasUpgrade("q", 24)) eff = eff.pow(4.25);
		...
		enEff2() { //HALVEDLOL
			...
			let exp = 5/9
			if (hasUpgrade("t", 35) && player.i.buyables[12].gte(4)) exp = .5605;
			let eff = player.t.energy.div(2).max(0).plus(1).log10().pow(exp);
			return softcap("timeEnEff2", eff).floor();
		},
		nextEnEff2() { //HALVEDLOL
			if (!hasUpgrade("t", 24)) return new Decimal(1/0);
			let next = Decimal.pow(5, reverse_softcap("timeEnEff2", tmp.t.enEff2.div(2).plus(1)).pow(1.4)).sub(1);
			return next;
		},
		...
		upgrades: {
			...
			11: { //HALVEDLOL
				...
				effect() { 
					return player.t.points.div(2).pow(0.9).add(0.25).plus(hasUpgrade("t", 13)?upgradeEffect("t", 13):0).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2:1);
				},
				effectDisplay() { return "+"+format(tmp.t.upgrades[11].effect) },
				formula() { 
					let f = "x/2^0.9"+(hasUpgrade("t", 13)?("+"+format(upgradeEffect("t", 13).plus(0.5))):"+0.25") 
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) f = "("+f+")^2"
					return f;
				},
			},
			12: { //HALVEDLOL
				title: "Limit Stretcher",
				description: "Time Energy cap starts later based on Boosters, and +0.5 Extra Time Capsule.",
				...
				effect() { 
					return player.b.points.div(2).pow(0.95).add(1)
				},
			...
			13: { //HALVEDLOL
				...
				effect() { 
					return player.t.buyables[11].div(2).add(tmp.t.freeExtraTimeCapsules).pow(0.95);
				},
			...
			14: { //HALVEDLOL
				title: "More Time",
				description: "The Time Energy effect is raised to the power of 1.15.",
			...
			21: { //HALVEDLOL
				...
				description: "The Time Energy limit is multiplied by 50.",
			...
			22: { //HALVEDLOL
				...
				unlocked() { return hasAchievement("a", 33) },
				effect() { 
					return player.e.points.div(2).plus(1).root(10).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.05:1); //HALVEDLOL
				},
				effectDisplay() { return format(tmp.t.upgrades[22].effect)+"x" },
				formula() { return "(x/2+1)^"+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"0.105":"0.1") },
			},
			23: {
			...
			25: { //HALVEDLOL
				...
				currencyLayer: "t",
				unlocked() { return hasAchievement("a", 33) },
				effect() { return player.t.energy.div(2).plus(1).log10().div(1.2).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2:1) },
				effectDisplay() { return "+"+format(tmp.t.upgrades[25].effect) },
				formula() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"(log(x/2+1)/1.2)^2":"log(x/2+1)/1.2" },
			},
			31: { //HALVEDLOL
				title: "Cheap Time",
				description: "Extra Time Capsule cost scaling is removed, and their cost exponent is decreased by 0.1.",
			...
			32: { //HALVEDLOL
				...
				description: "Hyperspace cost scales 16.67% slower.",
			...
			33: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.t.energy.div(2).plus(1).log10().plus(1).pow(2.25) },
				effectDisplay() { return format(tmp.t.upgrades[33].effect)+"x" },
				formula: "(log(x/2+1)+1)^2.25",
			},
			34: { //HALVEDLOL
				title: "Scalings Galore",
				description: "Post-1,225 Booster & Generator cost scalings start at 1,312 instead.",
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e39000":"e4240000") },
				currencyDisplayName: "time energy",
				currencyInternalName: "energy",
				currencyLayer: "t",
			...
			35: { //HALVEDLOL
				...
				description: "Time Energy's second effect exponent is increased (0.556 -> 0.5605)",
			...
			41: { //HALVEDLOL
				...
				description: "Raise the Subspace base to the power of 1.25, and gain 1,250x more Hyperspace Energy.",
		...
		freeExtraTimeCapsules() {
			...
			if (hasUpgrade("t", 12)) free = free.plus(0.5); //HALVEDLOL
		...
		buyables: {
			...
			11: {
				...
				costExp() {
					...
					if (hasUpgrade("t", 31) && player.i.buyables[12].gte(4)) exp = exp.sub(.1);
					return exp;
				},
...
addLayer("e", { //HALVEDLOL
        ...
        baseAmount() {return player.points}, // Get the current amount of baseResource
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?.0225:.02) }, // Prestige currency exponent //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
		...
		passiveGeneration() { return (hasMilestone("q", 1)&&player.ma.current!="e")?0.5:0 }, //HALVEDLOL
		update(diff) {
		...
		freeEnh() { //HALVEDLOL
			...
			if (hasUpgrade("e", 13)) enh = enh.plus(0.5); //HALVEDLOL
			if (hasUpgrade("e", 21)) enh = enh.plus(1); //HALVEDLOL
			if (hasUpgrade("e", 23)) enh = enh.plus(upgradeEffect("e", 23)); //HALVEDLOL
		...
		upgrades: { //HALVEDLOL
			...
			11: { //HALVEDLOL
				...
				effect() { 
					let exp = 1
					return {g: player.b.points.div(2).add(1).log10().pow(exp), b: player.g.points.div(2).add(1).log10().pow(exp)} 
				},
			...
			12: { //HALVEDLOL
				...
				effect() { 
					let ret = player.e.total.div(2).add(1).pow(1.25) 
					ret = softcap("e12", ret);
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) ret = ret.pow(1.25);
					return ret
				},
				effectDisplay() { return format(tmp.e.upgrades[12].effect)+"x" },
				formula() { 
					let f = upgradeEffect("e", 12).gte("1e1500")?"(x/2+1)^0.75*1e750":"(x/2+1)^1.25" 
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) f = "("+f+")^1.25"
					return f;
				},
			},
			13: { //HALVEDLOL
				title: "Enhance Half Plus",
				description: "Get a free half Enhancer.",
			...
			21: { //HALVEDLOL
				title: "Enhance Plus",
				description: "Get another free Enhancers",
			...
			23: { //HALVEDLOL
				...
				unlocked() { return hasAchievement("a", 33) },
				effect() {
					let eff = player.s.points.div(2).pow(1.5).div(25);
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) eff = eff.times(2.25);
					return eff.floor();
				},
				effectDisplay() { return "+"+formatWhole(tmp.e.upgrades[23].effect) },
				formula() { return "floor(x/2^1.5"+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"/11.11":"/25")+")" },
			},
			24: { //HALVEDLOL
				...
				unlocked() { return hasAchievement("a", 33) },
				effect() { return Decimal.pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e1000":1.05, player.b.points.div(2).plus(player.g.points.div(2)).pow(0.9)) },
				effectDisplay() { return format(tmp.e.upgrades[24].effect)+"x" },
				formula() { return (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e1,000":"1.05")+"^((boosters/2+generators/2)^0.9)" },
			},
			31: {
				title: "Amplification",
			...
			33: { //HALVEDLOL
				title: "Augmentation",
				description: "Both Enhancer effect exponents are 10% higher.",
				cost() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"e3460000":"e4500000") },
				pseudoUnl() { return player.i.buyables[12].gte(3)&&player.e.upgrades.length>=7 },
			...
			41: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.e.points.div(2).plus(1).log10().plus(1).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?.15:.075) },
				effectDisplay() { return format(tmp[this.layer].upgrades[this.id].effect)+"x" },
				formula() { return "(log(x/2+1)+1)^"+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"0.15":"0.075") },
			},
		},
		buyables: {
			rows: 1,
			...
			11: { //HALVEDLOL
				...
				power() {
					let pow = new Decimal(1);
					if (hasUpgrade("e", 33) && player.i.buyables[12].gte(3)) pow = pow.times(1.1); //HALVEDLOL
					return pow;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let power = tmp[this.layer].buyables[this.id].power
					x = x.div(2).plus(tmp.e.freeEnh);
					if (!unl(this.layer)) x = new Decimal(0);
					
                    let eff = {}
                    if (x.gte(0)) eff.first = Decimal.pow(12.5, x.pow(power.times(1.05))) //HALVEDLOL
                    else eff.first = Decimal.pow(1/12.5, x.times(-1).pow(power.times(1.05))) //HALVEDLOL
					if (hasUpgrade("q", 24)) eff.first = eff.first.pow(4.25); //HALVEDLOL
					eff.first = softcap("enh1", eff.first)
                
                    if (x.gte(0)) eff.second = x.pow(power.times(0.8)) //HALVEDLOL
                    else eff.second = x.times(-1).pow(power.times(0.8)).times(-1) //HALVEDLOL
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) eff.second = eff.second.pow(25); //I think I'm tripping over rn
                    return eff;
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                   ...
                   "+(tmp.nerdMode?(" Formula 1: 12.5^(x/2^"+format(data.power.times(1.05))+")\n\ Formula 2: x/2^"+format(data.power.times(0.8))):(" Boosts Prestige Point gain by " + format(data.effect.first) + "x and adds to the Booster/Generator base by " + format(data.effect.second)))+(inChallenge("h", 31)?("\nPurchases Left: "+String(10-player.h.chall31bought)):"")
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
...
addLayer("s", {
        ...
        baseAmount() {return player.points}, // Get the current amount of baseResource
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.625:1.85) }, // Prestige currency exponent //HALVEDLOL
        base() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2.5e6:(hasUpgrade("ss", 11)?5e12:1e15)) }, //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
            return mult
        },
		...
		space() { //HALVEDLOL
			let space = player.s.best.pow(1.05).times(2); //HALVEDLOL
			if (hasUpgrade("s", 13)) space = space.plus(1); //HALVEDLOL
			if (hasAchievement("a", 53)) space = space.plus(1); //HALVEDLOL
		...
		freeSpaceBuildings() {
			let x = new Decimal(0);
			if (hasUpgrade("s", 11)) x = x.plus(0.5);
		...
		manualBuildingLevels() { //HALVEDLOL
			let len = Object.keys(player.s.buyables).length
			if (len==0) return new Decimal(0);
		...
		upgrades: {
			...
			11: { //HALVEDLOL
				title: "Space X",
				description: "Add a free half level to all Space Buildings.",
			...
			12: { //HALVEDLOL
				...
				effect() { return player.g.power.div(2).add(1).log10().add(1) },
				effectDisplay() { return format(tmp.s.upgrades[12].effect)+"x" },
				formula: "log(x/2+1)+1",
			},
			13: { //HALVEDLOL
				title: "Shipped Away",
				description: "Space Building Levels boost Generator Power gain, and you get 1 extra Space.",
                ...
                currencyLayer: "g",
				unlocked() { return hasUpgrade("s", 11) },
				effect() { return softcap("s13", Decimal.pow(10, tmp.s.totalBuildingLevels.div(2))) },
			...
			15: { //HALVEDLOL
				title: "Four Square",
				description: "The <b>Quaternary Space Building</b> cost is square rooted, is 2x as strong, and also affects <b>BP Combo</b> (brought to the 2.7th root).",
			...
			21: { //HALVEDLOL
				title: "Spacious",
				description: "All Space Buildings are 4% stronger.",
			...
			22: { //HALVEDLOL
				...
				currencyLayer: "g",
				unlocked() { return hasAchievement("a", 33) },
				effect() { return player.t.points.div(2).cbrt().floor() },
				effectDisplay() { return "+"+formatWhole(tmp.s.upgrades[22].effect) },
				formula: "floor(cbrt(x/2))",
			},
			23: { //HALVEDLOL
				title: "Revert Space",
				description() { return (player.e.unlocked&&player.t.unlocked&&(player.s.unlockOrder||0)==0)?"All Space Building costs are divided by 1e10.":("Space acts as if you chose it first"+(player.t.unlocked?", and all Space Building costs are divided by 1e10.":".")) },
			...
			31: { //HALVEDLOL
				...
				description: "The first four Space Buildings' cost exponents are decreased by 0.02*(2.5-(n/2)), where n is the Space Building number.",
			...
			32: { //HALVEDLOL
				...
				description: "Each Space Building's bought Level adds to the previous building's Extra Half Level.",
			...
			33: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return upgradeEffect("s", 35).div(2).sqrt() },
			...
			34: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.s.points.div(2).plus(1).log10().plus(1).log10().plus(1) },
				effectDisplay() { return "brought to the "+format(tmp.s.upgrades[this.id].effect)+"th root" },
				formula: "log(log(x/2+1)+1)+1",
				style: {"font-size": "9px"},
			},
			35: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return tmp.s.space.div(2).plus(1) },
				effectDisplay() { return format(tmp.s.upgrades[this.id].effect)+"x" },
				formula: "x/2+1",
			},
		},
		divBuildCosts() { //HALVEDLOL
			let div = new Decimal(1);
			if (hasUpgrade("s", 23) && player.t.unlocked) div = div.times(1e10); //HALVEDLOL
			if (player.ss.unlocked) div = div.times(tmp.ss.eff3);
			return div;
		},
		buildScalePower() { //HALVEDLOL
			let scale = new Decimal(1);
			if (hasUpgrade("p", 42)) scale = scale.times(.75); //HALVEDLOL
			if (hasUpgrade("hn", 42)) scale = scale.times(.9); //HALVEDLOL
		...
		buyables: {
			...
			11: {
				...
				freeLevels() {
					...
					if (hasUpgrade("s", 32) && player.i.buyables[12].gte(5)) levels = levels.plus(player.s.buyables[11+1].div(2)||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
			...
			12: {
				...
				freeLevels() {
					let levels = tmp.s.freeSpaceBuildings.plus(tmp.s.freeSpaceBuildings1to4);
					if (hasUpgrade("s", 32) && player.i.buyables[12].gte(5)) levels = levels.plus(player.s.buyables[12+1].div(2)||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal //HALVEDLOL
					let eff = x.plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).div(2).sqrt();
					if (player.hs.unlocked) eff = eff.pow(buyableEffect("hs", 22));
					return eff;
                },
				display() { // Everything else displayed in the buyable button after the title //HALVEDLOL
                    let data = tmp[this.layer].buyables[this.id]
                    return (tmp.nerdMode?("Cost Formula: "+format(tmp.s.buildingBaseCosts[this.id])+"^((x*"+format(tmp.s.buildScalePower)+")^"+format(tmp[this.layer].buyables[this.id].costExp)+")*"+format(tmp.s.buildingBaseCosts[this.id])+"/"+format(tmp.s.divBuildCosts)):("Cost: " + formatWhole(data.cost) + " Generator Power"))+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
                    "+(tmp.nerdMode?("Formula: sqrt(level/2)"):("Adds to base of Booster/Generator effects by +" + format(data.effect)))
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
                    return player.g.power.gte(tmp[this.layer].buyables[this.id].cost) && layers.s.space().gt(0)},
			...
			13: {  //HALVEDLOL
				...
				freeLevels() {
					let levels = tmp.s.freeSpaceBuildings.plus(tmp.s.freeSpaceBuildings1to4);
					if (hasUpgrade("s", 32) && player.i.buyables[12].gte(5)) levels = levels.plus(player.s.buyables[13+1].div(2)||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let eff = Decimal.pow(1e9, x.div(2).plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).pow(0.9))  //HALVEDLOL
					if (player.hs.unlocked) eff = eff.pow(buyableEffect("hs", 23));  //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    "+(tmp.nerdMode?("Formula: "+(data.effect.gte("e3e9")?"10^((level^0.3)*5.45e6)":"1e9^(level/2^0.9)")):("Divide Booster/Generator cost by " + format(data.effect)))
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
                    return player.g.power.gte(tmp[this.layer].buyables[this.id].cost) && layers.s.space().gt(0)},
			...
			14: {
				...
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
					...
					if (hasUpgrade("s", 15)) cost = cost.root(2);
					return cost.div(tmp.s.divBuildCosts);
                },
				freeLevels() { //HALVEDLOL
					let levels = tmp.s.freeSpaceBuildings.plus(tmp.s.freeSpaceBuildings1to4);
					if (hasUpgrade("s", 32) && player.i.buyables[12].gte(5)) levels = levels.plus(player.s.buyables[14+1].div(2)||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal //HALVEDLOL
					let ret = x.plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).times((hasUpgrade("s", 15))?2:1).add(1).pow(1.125); //HALVEDLOL
					ret = softcap("spaceBuilding4", ret); //HALVEDLOL
					if (player.hs.unlocked) ret = ret.times(buyableEffect("hs", 24)); 
					return ret;
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
					let extForm = hasUpgrade("s", 15)?2:1
			...
			15: {
				...
				freeLevels() {
					...
					if (hasUpgrade("s", 32) && player.i.buyables[12].gte(5)) levels = levels.plus(player.s.buyables[15+1].div(2)||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
			...
			16: {
				...
				freeLevels() {
					...
					if (hasUpgrade("s", 32) && player.i.buyables[12].gte(5)) levels = levels.plus(player.s.buyables[16+1].div(2)||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
			...
			17: {
				...
				freeLevels() {
					let levels = new Decimal(0);
					if (hasUpgrade("s", 32) && player.i.buyables[12].gte(5)) levels = levels.plus(player.s.buyables[17+1].div(2)||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
			...
			18: {
				...
				freeLevels() {
					let levels = new Decimal(0);
					if (hasUpgrade("s", 32) && player.i.buyables[12].gte(5)) levels = levels.plus(player.s.buyables[18+1].div(2)||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
			...
			19: {
				...
				freeLevels() {
					let levels = new Decimal(0);
					if (hasUpgrade("s", 32) && player.i.buyables[12].gte(5)) levels = levels.plus(player.s.buyables[19+1].div(2)||0);
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
...
addLayer("sb", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
		branches: ["b"],
        exponent() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.1625:1.25 }, // Prestige currency exponent //HALVEDLOL
		base() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.0375:1.05 }, //HALVEDLOL
		gainMult() { 
			let mult = new Decimal(1);
		...
		effectBase() {
			...
			if (hasChallenge("h", 12)) base = base.plus(.125); //HALVEDLOL
			...
			if (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes('b'):false) && hasUpgrade("b", 12)) base = base.times(upgradeEffect("b", 12).max(1)); //HALVEDLOL
			if (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes('b'):false) && hasUpgrade("b", 13)) base = base.times(upgradeEffect("b", 13).max(1)); //HALVEDLOL
			base = base.times(tmp.n.dustEffs.blue);
			if (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("h"):false) && hasChallenge("h", 12)) base = base.times(player.hs.points.div(2).plus(1)); //HALVEDLOL
...
addLayer("sg", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
		branches: ["g"],
        exponent() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.2375:1.25 }, // Prestige currency exponent
		base() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.045:1.05 }, //HALVEDLOL
		gainMult() { 
			let mult = new Decimal(1);
			if (hasUpgrade("ss", 21)) mult = mult.div(1.2);
		...
		enEff() {
			...
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("sg"):false) eff = eff.pow(1.5); //HALVEDLOL
			return eff;
		},
		doReset(resettingLayer){ 
			let keep = []
...
addLayer("h", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?.2:.1625) }, // Prestige currency exponent //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
		...
		challenges: {
			...
			12: { //HALVEDLOL
				...
				currencyDisplayName: "points",
				currencyInternalName: "points",
				rewardDescription() { return "Add 0.125 to the Super Booster base"+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("h"):false)?(" and multiply it by your Hyperspace Energy"):"")+"." },
			},
			21: {
				name: "Out of Room",
				completionLimit: 1,
				...
				rewardEffect() { return player.s.points.div(4).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("h"):false)?1.2:1) },
				rewardDisplay() { return format(this.rewardEffect())+"% stronger (additive)" },
				formula() { return "(x/2*"+format(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("h"):false)?.7:.5)+")%" },
				onStart(testInput=false) {
					if (testInput) {
						resetBuyables("s");
			...
			31: {
				...
				rewardEffect() { 
					let eff = Decimal.div(5, Decimal.add((hasUpgrade("ss", 33)?(player.timePlayed||0):player.q.time.div(2)), 1).cbrt().pow(hasUpgrade("ss", 23)?(-1):1)).plus(1).pow(challengeCompletions("h", 31)).times(tmp.n.realDustEffs2?tmp.n.realDustEffs2.blueOrange:new Decimal(1)).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?3:1);
					if (!eff.eq(eff)) eff = new Decimal(1);
					return eff;
				},
				rewardDisplay() { return format(this.rewardEffect())+"x" },
				formula() { return "(5"+(hasUpgrade("ss", 23)?"*":"/")+"cbrt(time/2+1)+1)^completions" },
			},
			32: {
				name: "Option D",
				scalePower() {
			...
			42: {
				...
				currencyDisplayName: "points",
				currencyInternalName: "points",
				rewardDescription() { return "The Quirk Layer cost base is decreased by 0."+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("h"):false)?"1":"075")+", and unlock 2 new Subspace Upgrades." },
...
addLayer("q", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?.008:.00775) }, // Prestige currency exponent
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
		...
		buyables: {
			...
			11: {
				...
				costBase() {
					...
					if (hasChallenge("h", 42)) base = base.sub(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("h"):false)?.1:.075); //HALVEDLOL
					if (hasAchievement("a", 101)) base = base.sub(.1); //HALVEDLOL
					if (hasUpgrade("q", 25) && player.i.buyables[12].gte(6)) base = base.root(upgradeEffect("q", 25));
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) base = base.pow(.875); //HALVEDLOL
					return base;
				},
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
		...
		milestones: {
			...
			1: { //HALVEDLOL
				...
				effectDescription: "You can buy max Time & Space, gain 50% of Enhance Point gain every second, and unlock Auto-Enhancers & Auto-Extra Time Capsules.",
		...
		upgrades: {
			...
			35: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.i.hb.div(2).sqrt().div(50).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.25:1).plus(1) },
		...
		impr: {
			...
			free() { //HALVEDLOL
				let free = new Decimal(0);
				if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes('q'):false) free = free.plus(Decimal.div(player.s.buyables[20]||0, 8));
				return free.floor();
			},
...
addLayer("o", { //HALVEDLOL
	name: "solarity", // This is optional, only used in a few places, If absent it just uses the layer id.
        ...
        exponent() { 
			...
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) exp = exp.times(player.sb.points.div(2).times(0.25/100).plus(1)) //HALVEDLOL
		...
		effect() { //HALVEDLOL
			if (!unl(this.layer)) return new Decimal(0);
			let sol = player.o.points.div(2);
			sol = softcap("sol_eff", sol);
			let eff = sol.plus(1).log10();
			let cap = 0.1;
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) cap = 0.125; //HALVEDLOL
			if (eff.gt(10)) eff = eff.log10().times(2).plus(3.5)
			return eff.div(100).min(cap);
		},
		effect2() { return player.o.points.div(1e20).plus(1).sqrt().pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.05:1) }, //HALVEDLOL
		solEnGain() { 
		...
		solEnEff() { return Decimal.sub(2, Decimal.div(2, player.o.energy.plus(1).log10().plus(1))) }, //HALVEDLOL
		solEnEff2() { return player.o.energy.plus(1).pow(1.5) }, //HALVEDLOL
		...
		passiveGeneration() { return player.ma.current=="o"?0:(hasMilestone("m", 0)?1:(hasMilestone("o", 0)?0.025:0)) }, //HALVEDLOL
		solPow() { //HALVEDLOL
			let pow = new Decimal(1);
			...
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) pow = pow.plus(player.o.points.plus(1).log10().div(10)); //HALVEDLOL
		...
		buyableGainExp() {
			let exp = new Decimal(1);
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) exp = exp.times(1.8); //HALVEDLOL
			return exp;
		},
		buyables: { //HALVEDLOL
			rows: 3,
			...
			11: { //HALVEDLOL
				...
				effect() { 
					...
					return Decimal.pow(hasUpgrade("ss", 22)?(amt.div(2).plus(1).pow(tmp.o.solPow).cbrt()):(amt.div(2).plus(1).pow(tmp.o.solPow).log10().plus(1)), ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.05:1) //HALVEDLOL
				},
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
			...
			12: { //HALVEDLOL
				...
				effect() { return Decimal.pow(hasUpgrade("p", 24)?Decimal.pow(5, player[this.layer].buyables[this.id].div(2).times(tmp.o.multiplyBuyables).plus(1).log10().cbrt()):(player[this.layer].buyables[this.id].div(2).times(tmp.o.multiplyBuyables).plus(1).pow(tmp.o.solPow).log10().plus(1).log10().times(5).plus(1)), ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.05:1) },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
					...
					"Amount: " + formatWhole(player[this.layer].buyables[this.id])+((tmp.o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.o.multiplyBuyables))))+"\n"+
					(tmp.nerdMode?("Formula: "+(hasUpgrade("p", 24)?"5^cbrt(log(x/2+1))":"log(log(x/2+1)+1)*5+1")):("Effect: Multiplies the Super Booster base and each Quirk Layer by "+format(tmp[this.layer].buyables[this.id].effect)))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
			...
			13: { //HALVEDLOL
				...
				effect() { return player[this.layer].buyables[this.id].div(2).times(tmp.o.multiplyBuyables).plus(1).pow(tmp.o.solPow).log10().plus(1).pow(1.75).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?13.65:1) },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
					...
					"Amount: " + formatWhole(player[this.layer].buyables[this.id])+((tmp.o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.o.multiplyBuyables))))+"\n"+
					(tmp.nerdMode?("Formula: (log(x/2+1)+1)^1.75"):("Effect: Multiplies the Time Capsule base and Subspace gain by "+format(tmp[this.layer].buyables[this.id].effect)))
					return display;
                },
			...
			21: { //HALVEDLOL
				...
				effect() { 
					let eff = player[this.layer].buyables[this.id].div(2).times(tmp.o.multiplyBuyables).plus(1).pow(tmp.o.solPow).log10().plus(1).log10();
					eff = softcap("corona", eff);
					if (hasUpgrade("hn", 24)) eff = eff.times(1.5);
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) eff = eff.times(1.2);
					return eff;
				},
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
					...
					"Amount: " + formatWhole(player[this.layer].buyables[this.id])+((tmp.o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.o.multiplyBuyables))))+"\n"+
					(tmp.nerdMode?("Formula: log(log(x/2+1)+1)"):("Effect: +"+format(tmp[this.layer].buyables[this.id].effect)+" to Subspace base & +"+format(tmp[this.layer].buyables[this.id].effect.times(100))+"% Solar Power"))
					return display;
                },
			...
			22: { //HALVEDLOL
				...
				effect() {
					return player[this.layer].buyables[this.id].div(2).times(tmp.o.multiplyBuyables).plus(1).pow(tmp.o.solPow).log10().root(10).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.2:1).plus(1)
				},
				display() {
					let data = tmp[this.layer].buyables[this.id]
					...
					"Amount: "+formatWhole(player[this.layer].buyables[this.id])+((tmp.o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.o.multiplyBuyables)))+"\n"+
					(tmp.nerdMode?("Formula: (log(x/2+1)^0.1)+1"):("Effect: Multiply Honour gain (unaffected by softcap) and Purple, Blue, & Orange Dust gain by "+format(data.effect)+"x")))
				},
			...
			23: { //HALVEDLOL
				...
				effect() {
					return player[this.layer].buyables[this.id].div(2).times(tmp.o.multiplyBuyables).plus(1).pow(tmp.o.solPow).log10().plus(1).log10().root(2.5).div(2).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.2:1)
				},
				display() {
					let data = tmp[this.layer].buyables[this.id]
					...
					"Amount: "+formatWhole(player[this.layer].buyables[this.id])+((tmp.o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.o.multiplyBuyables)))+"\n"+
					(tmp.nerdMode?("Formula: (log(log(x/2+1)+1)^0.4)*50"):("Effect: Space Buildings are "+format(data.effect.times(100))+"% stronger")))
				},
			...
			31: { //HALVEDLOL
				...
				effect() {
					return player[this.layer].buyables[this.id].div(2).times(tmp.o.multiplyBuyables).plus(1).pow(tmp.o.solPow).log10().plus(1).log10().root(5).div(20).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.45:1)
				},
				display() {
					let data = tmp[this.layer].buyables[this.id]
					...
					"Req: 1e400 Solarity\n"+
					"Amount: "+formatWhole(player[this.layer].buyables[this.id])+((tmp.o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.o.multiplyBuyables)))+"\n"+
					(tmp.nerdMode?("Formula: (log(log(x/2+1)+1)^0.2)*5"):("Effect: Spells are "+format(data.effect.times(100))+"% stronger")))
				},
			...
			32: { //HALVEDLOL
				...
				effect() {
					return player[this.layer].buyables[this.id].div(2).times(tmp.o.multiplyBuyables).plus(1).pow(tmp.o.solPow).log10().plus(1).log10().plus(1).log10().div(1.6).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.45:1).plus(1)
				},
				display() {
					let data = tmp[this.layer].buyables[this.id]
					...
					"Amount: "+formatWhole(player[this.layer].buyables[this.id])+((tmp.o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.o.multiplyBuyables)))+"\n"+
					(tmp.nerdMode?("Formula: log(log(log(x/2+1)+1)+1)/1.6+1"):("Effect: Multiply the Solarity gain exponent by "+format(data.effect)+".")))
				},
			...
			33: { //HALVEDLOL
				...
				effect() {
					return player[this.layer].buyables[this.id].div(2).times(tmp.o.multiplyBuyables).plus(1).pow(tmp.o.solPow).log10().plus(1).log10().plus(1).log10().div(3).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.45:1);
				},
				display() {
					let data = tmp[this.layer].buyables[this.id]
					...
					"Req: 1e750 Solarity\n"+
					"Amount: "+formatWhole(player[this.layer].buyables[this.id])+((tmp.o.multiplyBuyables||new Decimal(1)).eq(1)?"":(" x "+format(tmp.o.multiplyBuyables)))+"\n"+
					(tmp.nerdMode?("Formula: log(log(log(x/2+1)+1)+1)/3"):("Effect: Add "+format(data.effect.times(100))+"% to Solar Power, Space Building Power, & Hyper Building Power.")))
				},
		...
		milestones: { //HALVEDLOL
			0: {
				...
				effectDescription: "Gain 2.5% of Solarity gain every second.",
			},
		},
})
...
addLayer("ss", {
        ...
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.085:1.1) }, // Prestige currency exponent //HALVEDLOL
		base() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.1:1.125) }, //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
		...
		effBase() { //HALVEDLOL
			...
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) base = base.times(Decimal.pow(1e5, player.ss.points)); //HALVEDLOL
			if (player.ne.unlocked) base = base.times(tmp.ne.thoughtEff2);
			
			if (hasUpgrade("t", 41) && player.i.buyables[12].gte(4)) base = base.pow(1.25);
			return base;
		},
		effect() { 
			if (!unl(this.layer)) return new Decimal(1);
		...
		eff1() { return player.ss.subspace.div(2).plus(1).pow(tmp.ss.effPow).log10().pow(2).times(50).floor() },
		eff2() { return player.ss.subspace.div(2).plus(1).pow(tmp.ss.effPow).log10().plus(1).log10().div(6) },
		eff3() { return player.ss.subspace.div(2).plus(1).pow(tmp.ss.effPow).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2e3:1e3) },
		tabFormat: ["main-display",
			"prestige-button",
			"resource-display",
			"blank",
		...
		upgrades: {
			...
			11: {
				...
				description: "The Space Energy cost base is reduced (1e15 -> 5e12).",
			...
			12: { //HALVEDLOL
				...
				effect() { 
					...
					if (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) && eff.gte(2)) eff = eff.sub(1).times(50).pow(2).div(1e6).plus(1);
					return eff;
				},
				effectDisplay() { return format(tmp.ss.upgrades[12].effect.sub(1).times(50))+"% stronger" },
				formula: "sqrt(x/2/2.5)*50",
			},
			13: { //HALVEDLOL
				...
				currencyLayer: "ss",
				unlocked() { return hasUpgrade("ss", 11) },
				effect() { return player.q.points.div(2).plus(1).log10().div(10).plus(1).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?200:1) },
				effectDisplay() { return format(tmp.ss.upgrades[13].effect)+"x" },
				formula: "log(x/2+1)/10+1",
			},
			21: {
			...
			31: { //HALVEDLOL
				...
				effect() { return tmp.s.space.div(2).plus(1).cbrt().sub(1).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.5:1).floor() }, //HALVEDLOL
				effectDisplay() { return "+"+formatWhole(tmp.ss.upgrades[31].effect) },
				formula: "cbrt(x/2+1)-1",
			},
			32: {
			...
			42: { //HALVEDLOL
				title: "Sub-Subspace",
				description: "Space Buildings are 50% stronger (additive).",
			...
			43: { //HALVEDLOL
				...
				endpoint() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"e1.25e8":"e1e6") },
				description() { return "When below "+format(tmp.ss.upgrades[43].endpoint)+", Point gain is raised to the power of 1.05. Otherwise, it is raised to the power of 1.005." },
...
addLayer("m", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.00775:0.007) }, // Prestige currency exponent //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
		...
		spellPower() { 
			...
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) power = power.plus(.25); //HALVEDLOL
			return power;
		},
		hexGain() { 
...
addLayer("ba", {
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.00875:0.005) }, // Prestige currency exponent //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1);
			if (hasAchievement("a", 74)) mult = mult.times(challengeEffect("h", 32));
		...
		upgrades: {
			...
			24: { //HALVEDLOL
				...
				effect() { 
					let ret = {
						pos: player.ba.neg.div(2e12).plus(1).log10().plus(1).pow(hasUpgrade("ba", 33)?5:2.5),
						neg: player.ba.pos.div(2e12).plus(1).log10().plus(1).pow(hasUpgrade("ba", 33)?5:2.5),
					} 
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) {
						ret.pos = Decimal.pow(10, ret.pos.log10().pow(1.25));
						ret.neg = Decimal.pow(10, ret.neg.log10().pow(1.25));
					}
					return ret;
				},
				effectDisplay() { return "Pos: "+format(tmp.ba.upgrades[24].effect.pos)+"x, Neg: "+format(tmp.ba.upgrades[24].effect.neg)+"x" },
				formula() { return "Pos: (log(neg/2e12+1)+1)^"+(hasUpgrade("ba", 33)?5:2.5)+", Neg: (log(pos/2e12+1)+1)^"+(hasUpgrade("ba", 33)?5:2.5) },
				style: {"font-size": "9px"},
			},
			31: {
			...
			33: { //HALVEDLOL
				title: "True Equality",
				description: "Both <b>Net Neutrality</b> effects are squared.",
...
addLayer("ps", {
		...
		base() { 
			let b = new Decimal("1e8000").root(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.5:1); //HALVEDLOL
        ...
        branches: ["q", ["h", 2]],
		soulGainExp() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.1:1.5 }, //HALVEDLOL
		soulGainMult() {
			let mult = new Decimal(1);
		...
		soulEffExp() {
			...
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) exp = exp.times(50); //HALVEDLOL
			return exp;
		},
		soulEff() {
		...
		buyables: {
			...
			11: {
				...
				scaleSlow() {
					...
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) speed = speed.times(1.5); //HALVEDLOL
					return speed;
				},
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
			...
			21: {
				...
				scaleSlow() {
					...
					if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) slow = slow.times(1.1); //HALVEDLOL
		...
		impr: {
			...
			power() { return tmp.ps.buyables[21].effect2.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.05:1) }, //HALVEDLOL
...
addLayer("hn", {
		...
		gainMult() { //HALVEDLOL
			...
			if (hasAchievement("a", 91)) mult = mult.times(1.05); //HALVEDLOL
		...
		upgrades: {
			...
			13: { //HALVEDLOL
				...
				effect() { return tmp.p.upgrades[13].effect.div(2).max(1).log10().plus(1).log10().times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?60:20).plus(1) },
				effectDisplay() { return "^"+format(tmp.hn.upgrades[13].effect) },
				formula: "log(log(x/2+1)+1)*20+1",
			},
			14: { //HALVEDLOL
				title: "Anti-Calm",
				description: "<b>Prestigious Intensity</b>'s effect is 2.5% stronger.",
				multiRes: [
					{
			...
			22: { //HALVEDLOL
				...
				effect() { return Decimal.pow(5, player.ps.souls.div(2).plus(1).log10().plus(1).log10().sqrt().times(3)).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?2:1) },
				effectDisplay() { return "^"+format(tmp.hn.upgrades[22].effect) },
				formula: "5^(sqrt(log(log(x/2+1)+1))*3)",
			},
			23: { //HALVEDLOL
				...
				effect() { return player.ba.points.div(2).plus(1).log10().plus(1).pow(.75).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?1.05:1) },
				effectDisplay() { return "^"+format(tmp.hn.upgrades[23].effect) },
				formula: "(log(x/2+1)+1)^0.75",
			},
			24: { //HALVEDLOL
				title: "Coronal Energies",
				description: "Both Coronal Wave effects are *1.5'd (unaffected by softcap).",
				multiRes: [
					{
			...
			31: { //HALVEDLOL
				...
				description: "Point gain is raised to the power of 1.025.",
				multiRes: [
					{
			...
			32: { //HALVEDLOL
				title: "Less Useless",
				description: "<b>Upgrade Power</b> is raised ^4.",
				multiRes: [
					{
			...
			33: { //HALVEDLOL
				...
				effect() { return Decimal.pow(5, player.hn.best.div(2).plus(1).log10().plus(1).log10().sqrt()).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?1.05:1) },
				effectDisplay() { return format(tmp.hn.upgrades[33].effect)+"x" },
				formula: "5^sqrt(log(log(x/2+1)+1))",
			},
			34: { //HALVEDLOL
				...
				effect() { return player.hn.total.div(2).plus(1).log10().plus(1).log10().plus(1).log10().plus(1) },
			...
			41: { //HALVEDLOL
				...
				effect() { return player.ps.power.div(2).plus(1).log10().plus(1).log10().times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?2.9:1.7).plus(1) },
				effectDisplay() { return "^"+format(tmp.hn.upgrades[41].effect) },
				formula: "log(log(x/2+1)+1)*1.7+1",
				style: {"font-size": "9px"},
			},
			42: { //HALVEDLOL
				title: "Spatial Awareness II",
				description: "Space Building costs scale 10% slower.",
				multiRes: [
					{
			...
			43: {
				...
				formula() { return "10^(log(quirkEnergyEff)^"+((hasUpgrade("t", 35) && player.i.buyables[12].gte(4))?"0.5605":"0.556")+")" },
			},
			44: {
...
addLayer("hs", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
			...
			if (hasUpgrade("t", 41) && player.i.buyables[12].gte(4)) mult = mult.times(1.25e3); //HALVEDLOL
			if (hasUpgrade("s", 33) && player.i.buyables[12].gte(5)) mult = mult.times(upgradeEffect("s", 33));
			if (player.ma.unlocked) mult = mult.times(tmp.ma.effect);
		...
		buildingPower() {
			...
			if (hasAchievement("a", 113)) pow = pow.plus(.05); //HALVEDLOL
		...
		buyables: {
			...
			11: {
				...
				scaleRate() {
					...
					if (hasUpgrade("t", 32) && player.i.buyables[12].gte(4)) rate = new Decimal(5/6);
			...
			22: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Secondary Space Building Effect: ^"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: level/2*20+1)":""))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
				...
				effect() {
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).div(2).times(20).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
			...
			23: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Tertiary Space Building Effect: ^"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: (level^0.8)*400+1)":""))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
				...
				effect() {
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).pow(0.8).times(400).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
			...
			24: {
				...
				display() { // Everything else displayed in the buyable button after the title //HALVEDLOL
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Quaternary Space Building Effect: x"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: (level^0.8)*2.5e3+1)":" (unaffected by softcap)"))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
				...
				effect() { //HALVEDLOL
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).pow(0.8).times(2.5e3).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
...
addLayer("ma", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
			if (hasAchievement("a", 131)) mult = mult.div(1.05); //HALVEDLOL
			if (hasAchievement("a", 95)) mult = mult.div(1.075); //HALVEDLOL
			if (hasAchievement("a", 134)) mult = mult.times(Decimal.pow(.9999625, player.ps.points)); //HALVEDLOL
			if (hasAchievement("a", 163)) mult = mult.div(Decimal.pow(1.05, player.a.achievements.filter(x => x>160).length)); //HALVEDLOL
            return mult
        },
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
		...
		rewardDesc() {
			let desc = "";
			if (player.ma.mastered.includes("p")) desc += "<h2>Prestige</h2><br><br><ul><li>Base Prestige gain exponent is better (0.5 -> 0.625)</li><li><b>Prestige Boost</b> is raised ^1.05 (after softcaps)</li><li><b>Self Synergy</b> is raised ^38</li><li><b>More Prestige</b> is much stronger (+40% -> +1e27%)</li><li><b>Upgrade Power</b> is raised ^20</li><li><b>Reverse Prestige Boost</b> is raised ^1.25</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("b")) desc += "<h2>Boosters</h2><br><br><ul><li>Booster cost base is reduced (5 -> 3.25)</li><li>Base Booster cost exponent is reduced (1.25 -> 1)</li><li><b>BP Combo</b> & <b>Discount One</b> are raised ^1.25</li><li><b>Cross Contamination</b> & <b>PB Reversal</b> also multiply the SB base</li><li><b>Worse BP Combo</b> is raised ^10,000</li><li><b>Even More Additions</b> is squared</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("g")) desc += "<h2>Generators</h2><br><br><ul><li>Generator cost base is reduced (5 -> 3.75)</li><li>Base Generator cost exponent is reduced (1.25 -> 1.175)</li><li>Generator Power effect is raised ^1.025</li><li><b>GP Combo</b> is raised ^250,000</li><li><b>I Need More III</b> is raised ^5,000</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("t")) desc += "<h2>Time</h2><br><br><ul><li>Time cost base is reduced (1e15 -> 1e8)</li><li>Base Time cost exponent is reduced (1.85 -> 1.4)</li><li>Time Capsules have a new effect</li><li>Anything that multiplies the Time Energy limit base now multiplies the Booster & Generator bases</li><li>The first Time Energy effect softcaps later (e3.1e9)</li><li>The Extra Time Capsule cost is raised ^0.9</li><li><b>Pseudo-Boost</b> & <b>Basic</b> also multiply the Time Energy gain base, and are both squared</li><li><b>Enhanced Time</b> is raised to the power of 1.05</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("e")) desc += "<h2>Enhance</h2><br><br><ul><li>Enhance gain exponent is increased (0.02 -> 0.0225)</li><li>The second Enhancer effect is raised to the power of 25</li><li><b>Enhanced Prestige</b> also affects Point gain, and it is raised ^1.25</li><li><b>Enter the E-Space</b> is 125% stronger</li><li><b>Monstrous Growth</b>'s base is much better (1.05 -> 1e1,000)</li><li><b>To the Next Level</b> is squared</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("s")) desc += "<h2>Space</h2><br><br><ul><li>Space cost base is reduced (5e12 -> 2.5e6)</li><li>Base Space cost exponent is reduced (1.85 -> 1.625)</li><li>Space Building Power is divided by 3.85, but Space Buildings cost scale 3x slower</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("sb")) desc += "<h2>Super Boosters</h2><br><br><ul><li>Super Booster cost base is reduced (1.05 -> 1.0375)</li><li>Base Super Booster cost exponent is reduced (1.25 -> 1.1625)</li><li>The Super Booster cost is divided by 1.167</li><li>Super Boosters provide Spectral Boosters</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("sg")) desc += "<h2>Super Generators</h2><br><br><ul><li>Super Generator cost base is reduced (1.05 -> 1.045)</li><li>Base Super Generator cost exponent is reduced (1.25 -> 1.2375)</li><li>The Super Generator cost is divided by 1.05</li><li>The Super Generator Power effect is ^1.5'd</li><li>Super Generators give Spectral Generators over time</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("q")) desc += "<h2>Quirks</h2><br><br><ul><li>Quirk gain exponent is increased (7.5e-3 -> 7.75e-3)</li><li>The Quirk Energy effect softcap start is raised ^1.125</li><li>The Quirk Layer cost base is raised ^0.875</li><li><b>Millennial Abilities</b> is 25% stronger</li><li>Bought Decary Space Building Levels add free Quirk Improvements (equal to the Level/8)</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("h")) desc += "<h2>Hindrances</h2><br><br><ul><li>Hindrance Spirit gain exponent is increased (0.125 -> 0.1625)</li><li>The Hindrance Spirit effect softcap is much weaker (exponent to the 4th root -> exponent to the 3.25th root)</li><li>Unlock a Hindrance milestone</li><li><b>Speed Demon</b> has a secondary effect</li><li><b>Out of Room</b>'s effect is 20% stronger</li><li><b>Timeless</b> & <b>Option D</b> no longer have a completion limit</li><li><b>Timeless</b>'s effect is raised ^3</li><li><b>Productionless</b>'s reduction to the Quirk Layer cost base is stronger (0.075 -> 0.1)</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("o")) desc += "<h2>Solarity</h2><br><br><ul><li>The Solarity gain exponent is increased by 0.25% for every Super Booster you have (additive)</li><li>The Solar Energy gain exponent limit is increased to 0.125, but beyond 0.1 it grows much slower</li><li>Solar Energy's second effect is 5% stronger</li><li>Solar Power is increased by 10% for every OoM of Solarity you have</li><li>Solarity buyable gain is raised ^1.8</li><li>All effects of the first row of Solarity buyables are raised ^1.05</li><li><b>Convectional Energy</b>'s effect is raised ^13</li><li>All effects of the second row of Solarity buyables are multiplied by 1.2</li><li>All effects of the third row of Solarity buyables are multiplied by 1.45</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("ss")) desc += "<h2>Subspace</h2><br><br><ul><li>The Subspace cost base is reduced (1.15 -> 1.125)</li><li>The base Subspace cost exponent is reduced (1.1 -> 1.085)</li><li>The Subspace base is multiplied by 1e5 for each Subspace Energy you have</li><li>The third Subspace effect is raised ^2</li><li>If <b>Subspatial Awakening</b>'s effect is above 100%, it is squared but divided by 10,000</li><li><b>Emissary of Smash</b>'s effect is raised ^200</li><li><b>No More Progress</b>'s effect is *1.5'd</li><li><b>Challenging Speedup</b>'s endpoint is much higher (e1,000,000 -> e1.25e8)</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("m")) desc += "<h2>Magic</h2><br><br><ul><li>The Magic gain exponent is increased (7e-3 -> 7.75e-3)</li><li>Add 25% to Spell Power</li><li>The Hex effect softcap does not apply to the boost to Hindrance Spirit, Quirk, and Solar Energy gain, but this effect is square rooted</li><li>The Hex effect softcap starts 5e-4% later for every OoM of Magic you have</li><li>The Hex effect softcap exponent is increased (10 -> 1e3)</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("ba")) desc += "<h2>Balance</h2><br><br><ul><li>The Balance Energy gain exponent is increased (5e-3 -> 8.75e-3)</li><li>There are no Positivity/Negativity nerfs</li><li>Both <b>Net Neutrality</b> effects have their exponents raised ^1.75</li><li><b>Visible Regeneration</b> is raised ^5</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("ps")) desc += "<h2>Phantom Souls</h2><br><br><ul><li>The Phantom Soul cost base is yroot 1.5'd</li><li>The base Damned Soul gain formula is improved (PS^1.5 -> 1.1^PS)</li><li>The Damned Soul effect is raised ^50</li><li>Wraiths cost scale 25% slower</li><li>Ghost Spirit cost scales 10% slower</li><li>Phantom Boosters are 5% stronger</li></ul><br><br>";
			if (player.ma.mastered.includes("hn")) desc += "<h2>Honour</h2><br><br><ul><li>The Honour gain exponent for its Balance Energy requirement is improved (0.02 -> 0.05)</li><li>Remove the softcap to the second Honour Upgrade</li><li><b>Self-Self-Synergy</b>'s effect is multiplied by 3</li><li><b>Point Efficiency</b> is maxed at 92% instead of 90%</li><li><b>Superpowered Upgrades</b>'s effect is doubled</li><li><b>Reversal Sensational</b> is 10% stronger</li><li><b>Column Leader Leader</b> is 10% stronger</li><li><b>Again and Again</b>'s effect is doubled</li><li><b>Quir-cursion</b>'s effect is raised ^50</li></ul><br><br>";
			...
			if (player.ma.mastered.includes("hs")) desc += "<h2>Hyperspace</h2><br><br><ul><li>The Hyper Building Limit requirement scales 20% slower</li><li>Add 0.1% to Hyper Building Power for every Hyperspace bought</li><li>The Hyper Building softcap starts 0.05 Levels later</li></ul><br><br>";
...
addLayer("ge", {
		...
		buyables: {
			...
			11: {
				...
				costDiv() {
					let div = new Decimal(1);
					if (hasAchievement("a", 124)) div = div.times(2); //HALVEDLOL
					return div;
				},
				free() {
					let free = new Decimal(0);
		...
		clickables: {
			...
			12: {
				...
				effectPer() { return Decimal.add(6, tmp.ge.buyables[11].effect).times(hasAchievement("a", 123)?2:1) }, //HALVEDLOL
				effect() { return Decimal.pow(tmp.ge.clickables[this.id].effectPer, player.ge.clickables[this.id]) },
...
addLayer("mc", {
		...
		clickables: {
			...
			21: {
				...
				effExp() { return hasAchievement("a", 133)?2:1 }, //HALVEDLOL
		...
		buyables: {
			...
			11: {
				...
				costDiv() { return new Decimal(hasAchievement("a", 132)?4:1) }, //HALVEDLOL
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    return x.div(10).plus(0.5).div(tmp[this.layer].buyables[this.id].costDiv).ceil();
                },
				buffExp() { 
					let exp = hasAchievement("a", 132)?7.5:2.5; //HALVEDLOL
		...
		challenges: {
			...
			11: {
				...
				gainMult() { 
					...
					if (hasAchievement("a", 143)) mult = mult.times(2); //HALVEDLOL
					if (hasMilestone("r", 0)) mult = mult.times(player.r.maxMinibots.max(1));
...
addLayer("id", {
		...
		base: new Decimal(1.2),                //HALVEDLOL   V
		effect() { return Decimal.sub((hasAchievement("a", 155)?0.0025:0)+(hasUpgrade("ai", 32)?0.99:0.95), Decimal.div(0.95, player.id.points.plus(1).log10().times(hasMilestone("id", 4)?1.5:1).times(hasMilestone("id", 5)?1.75:1).plus(1))) },
...
addLayer("c", {
        ...
        color: "#edb3ff",
        requires() { return Decimal.sub(108, hasAchievement("a", 164)?player.c.buyables[11].times(1):0).max(8) }, // Can be a function that takes requirement increases into account
...
addLayer("a", { //HALVEDLOL
        ...
        achievements: {
			...
			32: { //HALVEDLOL
				...
				tooltip: "Reach 1.8e308 Points. Reward: 1.5x Prestige Point gain.",
			...
			121: { //HALVEDLOL
				...
				tooltip() { return "Unlock Gears. Reward: Total Hyperspace makes the Hyper Building softcap start later"+(tmp.nerdMode?" (Formula: ((x/2)^0.2)/100)":" (Currently: +"+format(player.hs.buyables[11].div(2).root(5).times(.1))+")") },
			...
			124: { //HALVEDLOL
				...
				tooltip: "Reach e1,000,000 Honour. Reward: Gear Evolution requires 2x less Rotations, and is 10% stronger.",
			...
			132: { //HALVEDLOL
				...
				tooltip: "Reach a Shell size of at least 200m. Reward: Shell Expansion's buff is raised ^3, its cost is divided by 4, & you get 1 free Gear Evolutions.",
				image: "images/achs/132.png",
			},
			133: { //HALVEDLOL
				name: "Breaching the Barriers",
				done() { return player.mc.mechEn.times(tmp.mc.mechEnMult).gte("1e375") },
				tooltip: "Reach 1e375 Mech-Energy. Reward: You can have 2 parts of The Motherboard active at once, Northbridge's effect is squared, and there is a new Gear Upgrade.",
			...
			143: { //HALVEDLOL
				...
				tooltip: "Reach e5e11 Points while in The Brain. Reward: Double Signal gain",
			...
			152: { //HALVEDLOL
				...
				tooltip: "Reach e1e12 Generator Power. Reward: The Generator Power effect is raised ^1.2.",
			...
			155: { //HALVEDLOL
				...
				tooltip: "Reach 1,000 Thoughts. Reward: The Idea effect is increased by 0.0025.",
			...
			163: { //HALVEDLOL
				...
				tooltip() { return "Unlock Civilizations. Reward: Ideas multiply Artificial Consciousness gain, and divide the Mastery requirement by 1.05 for each achievement in this row and below (/"+format(Decimal.pow(1.05, player.a.achievements.filter(x => x>160).length))+")." },
				image: "images/achs/163.png",
			},
			164: { //HALVEDLOL
				name: "Existence was a Mistake",
				done() { return player.c.buyables[11].gte(1) },
				tooltip() { return "Get a Population of at least 1. Reward: Always have the first & fifth Idea milestones, and each Population decreases the Civilization requirement by 1 (-"+formatWhole(player.c.buyables[11].times(1).min(50))+", caps at -50)." },
